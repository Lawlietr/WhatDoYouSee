...
- [ ] Attempt 2 (planned): additive enrichment on the RESTORED prompt — keep the existing guidelines verbatim, then append: (a) a "go deeper" addendum (per-person appearance, personality/interests, income bracket, brands per item, culture/beliefs with visible-evidence-only rule, multi-person relationships, habits), (b) the conciseness rule, (c) grouped table keys `"Group: item"` as an OPTIONAL enhancement. Re-test on a strong backend before deploying; the grouped `DataTableView` (from attempt 1) is the ready-made renderer when the table format is re-introduced
