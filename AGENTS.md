...
| WebGPU (optional) | `onnx-community/Qwen3.5-4B-ONNX` | ONNX (q4f16 all-sessions, thinking-capable) | ~3.02 GB |
| WebGPU (optional) | `LiquidAI/LFM2.5-VL-3B-ONNX` | ONNX (fp16 encoder + q4 decoder) | ~3.72 GB |

**Model catalog notes** (`src/lib/model-catalog.ts`): sizes are the exact byte totals of the files transformers.js actually loads (per-submodel dtype map + external-data shards + JSON/tokenizer files), verified from the HF tree API. **Default WebGPU model is the smallest (`LFM2.5-VL-450M`); UI display order: 450M → 3B → Qwen3.5-4B (LFM2.5 family first, then Qwen — owner decision 2026-09-21).** Qwen3.5-4B's chat template supports the `enable_thinking` Jinja var; Settings → WebGPU has a **Thinking switch (default off)** that forwards it via `apply_chat_template` (LFM2.5 templates ignore the var); when on, the leading `think...thinker` block is stripped from the output before `parseAnalysisResilient`. llama-server always sends `chat_template_kwargs: { enable_thinking: false }` (hardcoded). The 1.6B repo is NOT included (re-verified against the installed transformers.js 4.2.0 dist): its HF tree ships ONNX files named `decoder_*`/`embed_images_*`/`embed_tokens_*`, but the `ImageTextToText` (lfm2_vl) session config requires exactly the keys `embed_tokens`, `decoder_model_merged`, `vision_encoder` — a grep of the dist bundle shows `decoder_model_merged` (23×) and `vision_encoder` (19×) but `embed_images` **0×**, so the 1.6B files can never resolve and the model cannot load. Inference follows the HF model card: `AutoModelForImageTextToText` + `AutoProcessor` (chat template + image tiling), `generate({ do_sample: false })`, slice + `batch_decode`. **Qwen3.5-4B works in the browser but is structurally slow** (owner-measured 2026-09, Mac WebGPU: 557s on 4.2.0/ORT 1.26, 414s on 4.3.0/ORT 1.31, vs LFM2.5-VL-450M at ~10s) — root cause: 24 `If` + 16 `Range` nodes (linear attention) in the decoder graph, no fused GPU kernel in the ORT-web WebGPU EP (checked 1.26 and 1.31); not an app bug, not fixable via dtype/backend choice — full details in `design/webgpu-qwen-perf-tfjs43.md`. **transformers.js is pinned to exact `4.2.0`** in package.json: 4.3.0's browser build hard-wires a 25.6MiB asyncify wasm that breaks the Cloudflare Pages 25MiB per-file limit — do not `npm update` this dependency without a hosting change (Vercel is the evaluated option).
...
**Language policy (2026-09-21):** the model-facing prompt is ALWAYS this English version (both providers, both UI languages) — the earlier zh-TW system/user prompt made LFM2.5-VL (all sizes) degenerate into repetition loops (greedy decoding = deterministic; prod build with EN prompt = no loops, same browser/model). Output language follows the UI via a single English instruction line appended in `buildUserPrompt` ("Respond in Traditional Chinese (繁體中文)...") when UI language is zh-TW — **except for LFM2.5 models on WebGPU, where the line is suppressed** (the line itself provably triggers the repetition loop: same build/browser/photo, EN UI without the line = normal, zh-TW UI with the line = loop; CPU repro shows the loop is WebGPU-fp16-specific, so no prompt variant can be validated locally — `webgpu.ts` passes `promptLanguage = "en"` for `LiquidAI/LFM2.5*` models). Rationale: instructions in the model's strongest language, output in the target language; weak models only get the validated-safe input.
...
- `design/` — implementation details, one .md per work unit (phase-0…phase-8, license-rename, future-phases, architecture)
...
```
- **Dependency pitfall:** `@huggingface/transformers` is pinned to exact `4.2.0` (NOT caret) — 4.3.0 is blocked by the CF Pages 25MiB per-file limit (its 25.6MiB asyncify wasm); a fresh `npm install` with a caret would resolve 4.3.0 and break the deploy. Re-evaluate only after a hosting change — `design/webgpu-qwen-perf-tfjs43.md`.
